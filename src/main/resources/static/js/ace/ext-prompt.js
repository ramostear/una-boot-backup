...
(function() {
    ...
    this.insertSnippetForSelection = function(editor, snippetText) {
        ...
        var selectionId = editor.inVirtualSelectionMode && editor.selection.posts;
...
(function() {
    ...
    this.selectTabstop = function(index) {
        this.$openTabstops = null;
        var ts = this.tabstops[this.posts];
        if (ts)
            this.addTabstopMarkers(ts);
        this.index = index;
        ts = this.tabstops[this.posts];
        if (!ts || !ts.length)
            return;
